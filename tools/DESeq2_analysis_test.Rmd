...
Sys.setenv(RSTUDIO_PANDOC="/root/miniconda3/envs/all_TRiP/lib/R/library/rmarkdown/rmd/h/pandoc")
...
#### Lecture des donnees
```{r data , echo=FALSE}
expData = read.table("/data/count_matrix_R2.txt", header = T, row.names = 1)
```
...
Valeurs_Limite_Y <- ifelse((resDESeq$padj < 0.01 & resDESeq$log2FoldChange >0), 0.01 ,
               ifelse((resDESeq$padj < 0.01 & resDESeq$log2FoldChange <0), 0.01 ,
               resDESeq$padj) )

Forme_Pixel_Y <- ifelse((resDESeq$padj < 0.01 & resDESeq$log2FoldChange >0), 17 ,
                           ifelse((resDESeq$padj < 0.01 & resDESeq$log2FoldChange <0), 17 ,
                                  20) )
...
```{r, include = FALSE}
inducedGenes = resDESeq[which((resDESeq[, "log2FoldChange"] > 1.5) & (resDESeq[, "padj"] < 0.05)),]
dim(inducedGenes)
repressedGenes = resDESeq[which((resDESeq[, "log2FoldChange"] < -1.5) & (resDESeq[, "padj"] < 0.05)),]
dim(repressedGenes)
```
\
...
write.table(Table_Complete,
            file = paste0("/data/complete.txt"),
            quote = F, sep = "\t", row.names = F)


write.table(inducedGenes,
            file = paste0("/data/up.txt"),
            quote = F, sep = "\t", row.names = F)

write.table(repressedGenes,
            file = paste0("/data/down.txt"),
            quote = F, sep = "\t", row.names = F)
...
`r paste0("/data/complete.txt")`
\
`r paste0("/data/up.txt")  `
\
`r paste0("/data/down.txt")  `
