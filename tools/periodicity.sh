...
if [ "${r}" = "metagene" ]; then
	...
	do
		...
		do
			...
			echo ${line} > ${O}${N}.${l}.ligneDatabaseTempo.periodicity.txt
			for pos in `seq 0 ${total}`;
			do
				data_temp_plus=$(mktemp /tmp/dataplusTempo.periodicity.XXX);
				data_temp_minus=$(mktemp /tmp/dataminusTempo.periodicity.XXX);
				gawk -v num=${pos} '{if($4=="+"){print $1":"$2+num":"$4}}' ${O}${N}.${l}.ligneDatabaseTempo.periodicity.txt | sort > ${data_temp_plus} ;
				...
				gawk -v num=${pos} '{if($4=="-"){print $1":"$2-num":"$4}}' ${O}${N}.${l}.ligneDatabaseTempo.periodicity.txt | sort > ${data_temp_minus} ;
		...
		done < ${O}${N}.${l}.databaseTempo.periodicity.txt
	done
fi
