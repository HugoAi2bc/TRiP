...
RUN echo "dkfjggfdfstdfsddfsgrskfg"
...
ENV PATH=/root/miniconda3/envs/all_TRiP_env/bin:$PATH
...
RUN conda init bash
RUN echo "conda activate all_TRiP_env" >> /root/.bashrc \
    && conda info --envs \
    && conda list --name all_TRiP_env
