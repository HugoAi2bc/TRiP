...
TRiP does not need installation (yipee) but a precise folder architecture is requisite (boo). 
...
- your genome fasta file: Whether it's the genome or the transcriptome, it must be your reference fasta file where reads will be aligned. It must, like the other files, be downloaded from the [Ensembl](ensembl.org) database.
...
### c) [configfile.yaml](link to configfile entre parenthèses) file
Configfile file is used to define parameters to tell TRiP how to process your data. It must be carefully completed and be present in the project directory everytime you want to run TRiP.
>Caution
>	Spaces and quotation marks **must not be changed**!
